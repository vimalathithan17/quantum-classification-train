...
for data_type in DATA_TYPES_TO_TRAIN:
    ...
    if data_type == 'Meth':
        ...
        pipeline = Pipeline([
            ('scaler', scaler),
            ('qml', MulticlassQuantumClassifierDataReuploadingDR(n_qubits=n_snv_features, n_layers=config['n_layers'], steps=config['steps'], n_classes=n_classes, verbose=args.verbose))
        ])
